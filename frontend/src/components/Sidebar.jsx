...
export default function Sidebar() {
  ...
  const handleLogout = async () => {
    try {
      await logout(); // clear session backend + reset user state di AuthContext
  ...
  if (onClick) {
    return (
      <div onClick={onClick} className="cursor-pointer">
        <div
          className={`relative flex items-center py-2.5 px-4 my-1 font-medium rounded-xl cursor-pointer transition-all duration-200 group ${
            active
        ...
        >
          ...
          {active && <div className="absolute left-0 w-1 h-6 bg-[#A3846B] rounded-r-full" />}
        </div>
      </div>
    );
  }
